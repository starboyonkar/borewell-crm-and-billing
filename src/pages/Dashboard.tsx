...
const Dashboard: React.FC = () => {
  ...
  const recentCustomers = [...customers].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 3);
  return <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-center">
        <h1 className="text-2xl md:text-3xl font-bold text-borewell-800">Dashboard</h1>
        <div className="flex space-x-4 mt-4 md:mt-0">
          <Link to="/add-customer">
            <Button className="bg-borewell-600 hover:bg-borewell-700 text-base font-bold">Order Now</Button>
          </Link>
          <Button variant="outline" className="border-borewell-600 text-borewell-600 hover:bg-borewell-50" onClick={() => window.print()}>
            Print Report
          </Button>
        </div>
      </div>
        ...
        <CardContent>
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              ...
              <tbody>
                {recentCustomers.map(customer => <tr key={customer.id} className="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    <td className="p-3">
                      <span className={`px-2 py-1 rounded-full text-xs ${customer.paymentStatus === 'Paid' ? 'bg-green-100 text-green-800' : customer.paymentStatus === 'Pending' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'}`}>
      ...
      {user?.role === 'admin' && <Card>
          ...
          <CardContent className="flex flex-wrap gap-4">
            ...
            <Button variant="outline" className="border-borewell-600 text-borewell-600 hover:bg-borewell-50" onClick={() => window.open('/customers', '_blank')}>
              View All Customers
            </Button>
          </CardContent>
        </Card>}
